...
Outside the Vite plugin, the root entry also exports functions for tooling
that needs to drive the compiler directly (editor integrations, the router's
build-time route scan, `@aihu/tsc`):

```ts
import { compileRouteMeta, compileToAst, transform } from '@aihu/compiler'

const { code } = transform(source, id, { target: 'universal' }) // compiled JS
const ast = compileToAst(source, id) // parsed SFC AST (SfcAst)
const route = compileRouteMeta(source, id) // `@route` metadata, or null
```

All three spawn (or reuse, via an in-process memo) the same `aihu-compile`
binary the Vite plugin uses internally.
